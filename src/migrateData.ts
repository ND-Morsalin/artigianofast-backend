  ...
  try {
    ...
    for (const user of users) {
      try {
        ...
        console.log(
          `⚠️ User ${user.username} might already exist, skipping...`
        );
      }
    }
    ...
    for (const client of clients) {
      try {
        ...
        console.log(
          `⚠️ Client ${client.name} might already exist, skipping...`
        );
      }
    }
    ...
    for (const jobType of jobTypes) {
      try {
        ...
        console.log(
          `⚠️ Job type ${jobType.name} might already exist, skipping...`
        );
      }
    }
    ...
    for (const activity of activities) {
      try {
        ...
        console.log(
          `⚠️ Activity ${activity.name} might already exist, skipping...`
        );
      }
    }
    ...
    for (const collaborator of collaborators) {
      try {
        ...
        console.log(
          `⚠️ Collaborator ${collaborator.name} might already exist, skipping...`
        );
      }
    }
    ...
    for (const jobActivity of jobActivities) {
      try {
        ...
        console.log(
          `⚠️ Job activity ${jobActivity.id} might already exist, skipping...`
        );
      }
    }
    ...
    for (const plan of subscriptionPlans) {
      try {
        ...
        console.log(
          `⚠️ Subscription plan ${plan.name} might already exist, skipping...`
        );
      }
    }
    ...
    for (const subscription of userSubscriptions) {
      try {
        ...
        console.log(
          `⚠️ User subscription ${subscription.id} might already exist, skipping...`
        );
      }
    }
    ...
    for (const sector of sectors) {
      try {
        ...
        console.log(
          `⚠️ Sector ${sector.name} might already exist, skipping...`
        );
      }
    }
    ...
    for (const webPage of webPages) {
      try {
        ...
        console.log(
          `⚠️ Web page ${webPage.title} might already exist, skipping...`
        );
      }
    }
    ...
    for (const spot of promotionalSpots) {
      try {
        ...
        console.log(
          `⚠️ Promotional spot ${spot.title} might already exist, skipping...`
        );
      }
    }
    ...
    console.log(
      "📝 All your data is now persisted in MySQL and will survive server restarts."
    );
  } catch (error) {
