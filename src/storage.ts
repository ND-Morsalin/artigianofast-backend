...
export interface IStorage {
  ...
  getPlanConfigurationByUser(
    userId: number
  ): Promise<PlanConfiguration | undefined>;
  createPlanConfiguration(
    config: InsertPlanConfiguration
  ): Promise<PlanConfiguration>;
  updatePlanConfiguration(
    id: number,
    config: Partial<InsertPlanConfiguration>
  ): Promise<PlanConfiguration | undefined>;
  ...
  updatePromotionalSpot(
    id: number,
    spot: Partial<InsertPromotionalSpot>
  ): Promise<PromotionalSpot | undefined>;
  ...
  updateCollaborator(
    id: number,
    collaborator: Partial<InsertCollaborator>
  ...
  updateJobActivity(
    id: number,
    jobActivity: Partial<InsertJobActivity>
  ): Promise<JobActivity | undefined>;
  ...
  assignCollaboratorToActivity(
    activityId: number,
    collaboratorId: number
  ): Promise<ActivityCollaborator>;
  removeCollaboratorFromActivity(
    activityId: number,
    collaboratorId: number
  ): Promise<boolean>;
}
...
export class MemStorage implements IStorage {
  ...
  async getActivePromotionalSpots(): Promise<PromotionalSpot[]> {
    ...
    console.log(
      "Recupero spot promozionali attivi, ora corrente:",
      currentTimeStr
    );
    ...
    return Array.from(this.promotionalSpots.values()).filter((spot) => {
      ...
      if (spot.timeRanges && spot.timeRanges.length > 0) {
        const timeRanges =
          typeof spot.timeRanges === "string"
            ? JSON.parse(spot.timeRanges)
            : spot.timeRanges;
        const hasValidTimeRange =
          ...
          timeRanges.some((range: any) => {
            ...
            return (
              currentTimeStr >= range.startTime &&
              currentTimeStr <= range.endTime
            );
          });
        ...
        if (currentTimeStr < spot.startTime || currentTimeStr > spot.endTime)
          return false;
      }
  ...
  async createPromotionalSpot(
    spot: InsertPromotionalSpot
  ): Promise<PromotionalSpot> {
  ...
  async updatePromotionalSpot(
    ...
    const updatedSpot: PromotionalSpot = {
      ...
      textAnimationType:
        spot.textAnimationType ?? existingSpot.textAnimationType,
      ...
      startDate: spot.startDate
        ? new Date(spot.startDate)
        : existingSpot.startDate,
  ...
  constructor() {
    ...
    this.createPromotionalSpot({
      ...
      images: JSON.stringify([
        "https://via.placeholder.com/300x100?text=Demo+Spot",
      ]),
      startDate: new Date(),
    ...
    return Array.from(this.jobs.values()).filter((job) => {
      ...
      // Caso 3: Il lavoro non ha endDate ma ha una durata definita
        ...
        calculatedEndDate.setHours(
          calculatedEndDate.getHours() + Math.floor(parseFloat(job.duration))
        );
        calculatedEndDate.setMinutes(
          calculatedEndDate.getMinutes() + (parseFloat(job.duration) % 1) * 60
        );
  ...
  async createJob(insertJob: InsertJob): Promise<Job> {
    ...
    if (!endDate && insertJob.startDate && insertJob.duration) {
      ...
      endDate.setHours(
        endDate.getHours() + Math.floor(parseFloat(insertJob.duration))
      );
      endDate.setMinutes(
        endDate.getMinutes() + (parseFloat(insertJob.duration) % 1) * 60
      );
    }
  ...
  async updateJob(
    ...
    if ((jobUpdate.startDate || jobUpdate.duration) && !jobUpdate.endDate) {
      ...
      if (startDate && duration) {
        ...
        endDate.setMinutes(
          endDate.getMinutes() + (parseFloat(duration) % 1) * 60
        );
  ...
  async updateJobType(
    id: number,
    jobTypeUpdate: Partial<InsertJobType>
  ): Promise<JobType | undefined> {
      ...
      if (activity.jobTypeIds) {
        try {
          ...
          if (activity.jobTypeIds.includes(",")) {
            const jobTypeIds = activity.jobTypeIds
              .split(",")
          ...
          if (
            activity.jobTypeIds.startsWith("[") &&
            activity.jobTypeIds.endsWith("]")
          ) {
    ...
    return Array.from(this.activities.values()).filter((activity) => {
      ...
      if (activity.jobTypeIds) {
        try {
          ...
          // Gestione stringa CSV
          if (activity.jobTypeIds.includes(",")) {
            activityJobTypeIds = activity.jobTypeIds
              .split(",")
              .map((id) => parseInt(id.trim()))
              .filter((id) => !isNaN(id));
          }
  ...
  async createCollaborator(
    insertCollaborator: InsertCollaborator
  ...
  async updateCollaborator(
    id: number,
    collaboratorUpdate: Partial<InsertCollaborator>
  ): Promise<Collaborator | undefined> {
  ...
  async createJobActivity(
    insertJobActivity: InsertJobActivity
  ): Promise<JobActivity> {
  ...
  async updateJobActivity(
    id: number,
    jobActivityUpdate: Partial<InsertJobActivity>
  ): Promise<JobActivity | undefined> {
  ...
  async assignCollaboratorToActivity(
    activityId: number,
    collaboratorId: number
  ): Promise<ActivityCollaborator> {
  ...
  async removeCollaboratorFromActivity(
    activityId: number,
    collaboratorId: number
  ): Promise<boolean> {
    const key = `${activityId}:${collaboratorId}`;
  ...
  async updateSubscriptionPlan(
    id: number,
    planUpdate: Partial<InsertSubscriptionPlan>
  ): Promise<SubscriptionPlan | undefined> {
  ...
  async updateUserSubscription(
    id: number,
    subscriptionUpdate: Partial<InsertUserSubscription>
  ): Promise<UserSubscription | undefined> {
  ...
  async updateSector(
    id: number,
    sectorUpdate: Partial<InsertSector>
  ): Promise<Sector | undefined> {
  ...
  async getWebPagesByType(type: string): Promise<WebPage[]> {
    return Array.from(this.webPages.values()).filter(
      (page) => page.type === type
    );
  }
  ...
  async updateWebPage(
    id: number,
    webPageUpdate: Partial<InsertWebPage>
  ): Promise<WebPage | undefined> {
    try {
      ...
      if (webPageUpdate.isHomepage) {
        for (const [pageId, page] of Array.from(this.webPages.entries())) {
          if (
            page.type === (webPageUpdate.type || existingPage.type) &&
            page.isHomepage &&
            pageId !== id
          ) {
            page.isHomepage = false;
      ...
      console.error(
        `Errore durante l'aggiornamento della pagina web con ID ${id}:`,
        error
      );
      return undefined;
    }
  ...
  async deleteWebPage(id: number): Promise<boolean> {
    try {
      ...
      console.error(
        `Errore durante l'eliminazione della pagina web con ID ${id}:`,
        error
      );
      return false;
    }
  ...
  async getPlanConfigurationByUser(
    userId: number
  ): Promise<PlanConfiguration | undefined> {
  ...
  async createPlanConfiguration(
    insertConfig: InsertPlanConfiguration
  ): Promise<PlanConfiguration> {
  ...
  async updatePlanConfiguration(
    id: number,
    configUpdate: Partial<InsertPlanConfiguration>
  ): Promise<PlanConfiguration | undefined> {
    ...
    if (existing) {
      ...
      const newSettings: GeneralSettings = {
        ...
        enableWhatsAppNotifications:
          settings.enableWhatsAppNotifications || null,
        ...
        passwordRequireSpecialChars:
          settings.passwordRequireSpecialChars ?? null,
