...
export class MySQLStorage implements IStorage {
  ...
  async deleteClient(id: number): Promise<boolean> {
    ...
    console.log(
      `[DB] deleteClient id=${id} affected=${result[0].affectedRows}`
    );
    return ok;
  }
  ...
  async getJobsByDateRange(startDate: Date, endDate: Date): Promise<Job[]> {
    ...
    return await this.db
      .select()
      .from(jobs)
      .where(and(eq(jobs.startDate, startDate), eq(jobs.endDate, endDate)));
  }
  ...
  async getActivitiesByJobType(jobTypeId: number): Promise<Activity[]> {
    ...
    return await this.db
      .select()
      .from(activities)
      .where(eq(activities.jobTypeId, jobTypeId));
  }
  ...
  async getCollaboratorsByRole(roleId: number): Promise<Collaborator[]> {
    ...
    return await this.db
      .select()
      .from(collaborators)
      .where(eq(collaborators.roleId, roleId));
  }
  ...
  async createCollaborator(
    ...
    const newCollaborator = insertId
      ? await this.getCollaborator(insertId)
      : undefined;
  ...
  async getCollaboratorsByActivity(activityId: number): Promise<number[]> {
    ...
    const result = await this.db
      .select()
      .from(activityCollaborators)
      .where(eq(activityCollaborators.activityId, activityId));
  ...
  async getActivitiesByCollaborator(collaboratorId: number): Promise<number[]> {
    ...
    const result = await this.db
      .select()
      .from(activityCollaborators)
      .where(eq(activityCollaborators.collaboratorId, collaboratorId));
  ...
  async assignCollaboratorToActivity(
    ...
    await this.ensureInitialized();
    const result = await this.db
      .insert(activityCollaborators)
      .values({ activityId, collaboratorId });
    const newAssignment = await this.db
      ...
      .limit(1);
    if (!newAssignment[0])
      throw new Error("Failed to assign collaborator to activity");
  ...
  async createSubscriptionPlan(
    ...
    let insertId = Array.isArray(result)
      ? (result[0] as any)?.insertId
      : (result as any)?.insertId;
    ...
    const newPlan = insertId
      ? await this.getSubscriptionPlan(insertId)
      : undefined;
  ...
  async getPromotionalSpot(id: number): Promise<PromotionalSpot | undefined> {
    ...
    const result = await this.db
      .select()
      .from(promotionalSpots)
      .where(eq(promotionalSpots.id, id))
      .limit(1);
    return result[0];
  }
  ...
  async createPromotionalSpot(
    ...
    if (!insertId) {
      try {
        const [last] = await this.db
          .select()
          .from(promotionalSpots)
          .orderBy(desc(promotionalSpots.id))
          .limit(1);
        if (last?.id) insertId = last.id;
      } catch (e) {
    ...
    const newSpot = insertId
      ? await this.getPromotionalSpot(insertId)
      : undefined;
  ...
  async getPlanConfigurationByUser(
    ...
    const result = await this.db
      .select()
      .from(planConfigurations)
      .where(eq(planConfigurations.userId, userId))
      .limit(1);
    return result[0];
  }
    ...
    if (existing) {
      ...
      return {
        ...
        enableWhatsAppNotifications:
          newSettings.enableWhatsAppNotifications ?? null,
        defaultNotificationTime:
          (newSettings as any).defaultNotificationTime ?? null,
        ...
        passwordRequireNumbers:
          (newSettings as any).passwordRequireNumbers ?? null,
        passwordRequireSpecialChars:
          (newSettings as any).passwordRequireSpecialChars ?? null,
