...
router.get("/user-behavior", async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      error: "Errore nel recupero delle metriche di comportamento degli utenti",
    });
  }
});
...
router.get("/features", async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Errore nel recupero delle analisi delle funzionalità" });
  }
});
...
router.get("/performance", async (req: Request, res: Response) => {
  try {
    const performanceMetrics =
      await AnalyticsService.getSystemPerformanceMetrics();
    res.json(performanceMetrics);
  } catch (error) {
    console.error("Error fetching performance metrics:", error);
    res
      .status(500)
      .json({ error: "Errore nel recupero delle metriche di prestazioni" });
  }
});
