...
router.get("/", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Errore nel recupero degli spot promozionali" });
  }
});
...
router.get("/active", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Errore nel recupero degli spot promozionali attivi" });
  }
});
...
router.get("/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Errore nel recupero dello spot promozionale" });
  }
});
...
router.post("/", async (req, res) => {
  try {
    console.log(
      "Ricevuta richiesta POST per nuovo spot promozionale:",
    ...
    if (Array.isArray(req.body.visiblePages)) {
      req.body.visiblePages =
        req.body.visiblePages.length === 0
          ? "all"
          : JSON.stringify(req.body.visiblePages);
    } else if (req.body.visiblePages && req.body.visiblePages.length === 0) {
      req.body.visiblePages = "all";
    }

    console.log(
      "Dati spot promozionale processati:",
      JSON.stringify(req.body, null, 2)
    );
    ...
    console.log(
      "Dati spot promozionale validati, creazione in corso...",
    ...
    res
      .status(500)
      .json({ error: "Errore nella creazione dello spot promozionale" });
  }
});
...
router.patch("/:id", async (req, res) => {
  try {
    ...
    console.log(
      `Ricevuta richiesta PATCH per spot ID ${id}:`,
      JSON.stringify(req.body, null, 2)
    );
    ...
    console.log(
      "Dati spot promozionale processati per aggiornamento:",
      JSON.stringify(req.body, null, 2)
    );
    ...
    res
      .status(500)
      .json({ error: "Errore nell'aggiornamento dello spot promozionale" });
  }
});
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (success) {
      ...
      res
        .status(500)
        .json({ error: "Errore nell'eliminazione dello spot promozionale" });
    }
  } catch (error) {
