...
async function main() {
  ...
  const isAutoIncrement =
    Array.isArray(idColRows) &&
    idColRows[0] &&
    String(idColRows[0].EXTRA || "")
      .toLowerCase()
  ...
  if ((idxRows[0]?.cnt || 0) === 0) {
    ...
    if (dt === "text") {
      await conn.query(
        "CREATE UNIQUE INDEX `users_username_unique` ON `users` (`username`(255))"
      );
    } else {
      await conn.query(
  ...
  console.log(
    "✅ users table verified/fixed (engine, AUTO_INCREMENT, PK, unique username)."
  );
