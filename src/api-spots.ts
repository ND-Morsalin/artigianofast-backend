...
export function registerMobileSpotEndpoints(app: Express) {
  ...
  app.get("/api/spots", async (req, res) => {
    try {
      ...
      console.log(
        "API Spots - Spot promozionali attivi:",
        JSON.stringify(activeSpots, null, 2)
      );
      ...
      if (!activeSpots || activeSpots.length === 0) {
        console.log(
          "API Spots - Nessuno spot attivo trovato nel database. Creazione spot demo per test..."
        );
        ...
        console.log(
          "API Spots - Spot demo creato:",
          JSON.stringify(demoSpot, null, 2)
        );
      ...
      const spotsWithFullImageUrls = spotsToReturn.map((spot) => {
        ...
        if (Array.isArray(spot.images)) {
          ...
          const fullImages = (spot.images as any)
            .map((img: any) => {
              if (!img) return "";
      ...
      console.log(
        "API Spots - Spot promozionali da restituire:",
        JSON.stringify(spotsWithFullImageUrls, null, 2)
      );
      ...
      console.error(
        "API Spots - Errore nel recupero degli spot promozionali attivi:",
        error
      );
      res
        .status(500)
