...
export async function registerRoutes(app: Express): Promise<Server> {
      ...
      case "payment_intent.succeeded":
        ...
        if (paymentIntent.metadata?.userId && paymentIntent.metadata?.planId) {
          try {
            ...
            const subscription = await storage.getUserSubscriptionByUserId(
  ...
  app.post("/api/checkout/subscribe", async (req: Request, res: Response) => {
    try {
      ...
      if (!email || !fullName || !planId) {
        return res
          .status(400)
          .json({ error: "Email, nome completo e piano sono richiesti" });
      }
      ...
      if (!isFreePlan && !password) {
        return res
          .status(400)
          .json({ error: "Password richiesta per piani a pagamento" });
      }
      ...
      if (existingUser) {
        ...
        if (!password) {
          return res
            .status(400)
            .json({ error: "Password richiesta per creare un nuovo utente" });
        }
      ...
      if (!isFreePlan) {
        ...
        if (paymentMethod === "creditcard" && payment) {
          ...
          if (!cardNumber || !expiryDate || !cvc || !nameOnCard) {
            return res
              .status(400)
              .json({ error: "Dati carta di credito incompleti" });
          }
          ...
          try {
            ...
            const paymentIntent = await stripe.paymentIntents.create({
              ...
              confirm: true,
              return_url: `${
                process.env.APP_URL || "https://artigianofast.com"
              }/desktop/payment-success`,
              metadata: {
                email: email,
      ...
      return res
        .status(500)
        .json({ error: "Errore durante la registrazione e l'abbonamento" });
    }
  });
  ...
  app.put("/api/clients/:id", async (req: Request, res: Response) => {
    try {
      ...
      if (userId) {
        ...
        const planConfig =
          await PlanEnforcementService.getUserPlanConfiguration(userId);

        // Check client edit permission
        const canEditClients =
          planConfig?.features?.permissions?.edit_clients !== false;
        if (!canEditClients) {
    ...
    try {
      ...
      if (userId) {
        ...
        const canDeleteClients =
          planConfig?.features?.permissions?.delete_clients !== false;
        if (!canDeleteClients) {
          return res.status(403).json({
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!subscription) {
          return res
            .status(404)
            .json({ message: "No subscription found for this client" });
        }
        ...
        return res
          .status(500)
          .json({ message: "Error fetching client subscription" });
      }
    }
  );
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        return res
          .status(500)
          .json({ message: "Error updating client subscription" });
      }
    }
  );
  ...
  app.post("/api/collaborators", async (req: Request, res: Response) => {
    try {
      ...
      const activationToken =
        collaboratorData.activationToken ||
      ...
      try {
        const baseUrl =
          process.env.APP_PUBLIC_URL || "https://artigianofast.com";
        const activationUrl = `${baseUrl}/activate?token=${activationToken}`;
        const { notifyCollaboratorActivation } = await import(
          "./services/notifications"
        );
        await notifyCollaboratorActivation(
          newCollaborator as any,
          activationUrl
        );
      } catch (e) {
        console.log(
          `Failed to send collaborator activation: ${String(e)}`,
          "notifications"
        );
      }
    ...
    try {
      ...
      (async () => {
        try {
          ...
          console.log(
            `Failed to send job created notifications: ${String(e)}`,
            "notifications"
          );
        }
      })();
  ...
  app.post(
    "/api/dev/run-job-reminders",
    async (_req: Request, res: Response) => {
      try {
        const { sendUpcomingJobReminders } = await import(
          "./services/notifications"
        );
  ...
  app.post(
    "/api/dev/run-renewal-reminders",
    async (_req: Request, res: Response) => {
      try {
        const { processPlanRenewalReminders } = await import(
          "./services/notifications"
        );
  ...
  app.put("/api/jobtypes/:id", async (req: Request, res: Response) => {
    try {
      ...
      console.log("🔍 Validated job type data:", jobTypeData);
      const updatedJobType = await storage.updateJobType(
        Number(req.params.id),
        jobTypeData
      );
      if (!updatedJobType) {
  ...
  app.get(
    "/api/jobtypes/:jobTypeId/activities",
    async (req: Request, res: Response) => {
  ...
  app.post("/api/activities", async (req: Request, res: Response) => {
    try {
      ...
      if (
        activityData.jobTypeIds &&
        typeof activityData.jobTypeIds === "string"
      ) {
        try {
          ...
          if (Array.isArray(parsedJobTypeIds)) {
            for (const jobTypeId of parsedJobTypeIds) {
              ...
              if (!jobType) {
                return res
                  .status(400)
                  .json({ message: `Job type with ID ${jobTypeId} not found` });
              }
            }
          }
        } catch (err) {
          ...
          console.log(`Error parsing jobTypeIds: ${err}`, "activities");
        }
      }
      ...
      console.log(`Error creating activity: ${error}`, "activities");
  ...
  app.put("/api/activities/:id", async (req: Request, res: Response) => {
    try {
      ...
      // Log the activity data for debugging
      console.log(
        `Updating activity ${req.params.id}: ${JSON.stringify(activityData)}`,
        "activities"
      );
      ...
      console.log(`Error updating activity: ${error}`, "activities");
  ...
  app.post("/api/roles", async (req: Request, res: Response) => {
    try {
      ...
      if (permissions) {
        ...
        if (typeof permissions === "string") {
          try {
            ...
            console.log(`Failed to parse permissions JSON: ${err}`, "roles");
            return res
              .status(400)
              .json({ message: "Invalid permissions format" });
          }
        }
        ...
        if (!permissions.every((p: any) => typeof p === "string")) {
          console.log(
            `Invalid permission types: ${JSON.stringify(permissions)}`,
            "roles"
          );
          return res
            .status(400)
            .json({ message: "All permissions must be strings" });
        }
      }
      ...
      if (restData.sectorId !== undefined && restData.sectorId !== null) {
        restData.sectorId =
          restData.sectorId === "" ? null : Number(restData.sectorId);
      }
  ...
  app.put("/api/roles/:id", async (req: Request, res: Response) => {
    try {
      ...
      // Log the incoming data for debugging
      console.log(
        `Updating role ${req.params.id} with data: ${JSON.stringify(req.body)}`,
        "roles"
      );
      ...
      if (permissions !== undefined) {
        ...
        if (
          permissions &&
          !permissions.every((p: any) => typeof p === "string")
        ) {
          console.log(
            `Invalid permission types: ${JSON.stringify(permissions)}`,
            "roles"
      ...
      const updatedRole = await storage.updateRole(
        Number(req.params.id),
        roleData
      );
      if (!updatedRole) {
  ...
  app.get("/api/collaborators", async (req: Request, res: Response) => {
    ...
    const collaboratorsWithRoles = await Promise.all(
      collaborators.map(async (collaborator) => {
        // Get primary role
        const primaryRole = collaborator.roleId
          ? await storage.getRole(collaborator.roleId)
          : null;
        ...
        if (collaborator.roleIds) {
          try {
            ...
            const roleIdsArray =
              typeof collaborator.roleIds === "string"
            ...
            console.log(
              `Error parsing roleIds for collaborator ${collaborator.id}: ${err}`,
              "collaborators"
            );
          }
        }
  ...
  app.get(
    "/api/roles/:roleId/collaborators",
    async (req: Request, res: Response) => {
      ...
      const collaborators = allCollaborators.filter((c) => {
        ...
        if (c.roleIds) {
          try {
            const roleIdsArray =
              typeof c.roleIds === "string" ? JSON.parse(c.roleIds) : c.roleIds;
            ...
            console.log(
              `Error parsing roleIds for collaborator ${c.id}: ${err}`,
              "collaborators"
            );
          }
        }
  ...
  app.post("/api/collaborators", async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `Creating collaborator with data: ${JSON.stringify(req.body)}`,
        "collaborators"
      );
      ...
      try {
        ...
        console.log(`Error parsing roleIds: ${e}`, "collaborators");
      }
      ...
      if (collaboratorData.roleIds) {
        try {
          ...
          console.log(`Error validating roleIds: ${err}`, "collaborators");
          return res
            .status(400)
            .json({ message: "Formato dei ruoli non valido" });
        }
      } else {
        return res.status(400).json({
          message: "È necessario selezionare almeno un ruolo",
      ...
      if (collaboratorData.email && collaboratorData.notifyByEmail) {
        console.log(
          `MOCK EMAIL to ${collaboratorData.email}: Invito a unirti come collaboratore di ${newCollaborator.name}. Usa questo link per attivare il tuo account: ${activationLink}`,
          "collaborators"
        );
      ...
      if (collaboratorData.phone && collaboratorData.notifyByWhatsApp) {
        console.log(
          `MOCK WHATSAPP to ${collaboratorData.phone}: Sei stato invitato come collaboratore. Usa questo link per attivare il tuo account: ${activationLink}`,
          "collaborators"
        );
        notificationSent = true;
      }
      ...
      if (error instanceof ZodError) {
        const validationError = fromZodError(error);
        console.log(
          `Collaborator validation error: ${validationError.message}`,
          "collaborators"
        );
        return res.status(400).json({ message: validationError.message });
      }
      console.log(`Error creating collaborator: ${error}`, "collaborators");
  ...
  app.put("/api/collaborators/:id", async (req: Request, res: Response) => {
    try {
      // Log incoming data for debugging
      console.log(
        `Updating collaborator ${req.params.id} with data: ${JSON.stringify(
          req.body
        )}`,
        "collaborators"
      );
      ...
      if (collaboratorData.roleIds) {
        ...
        const existingCollaborator = await storage.getCollaborator(
          Number(req.params.id)
        );
      ...
      if (updatedCollaborator.roleIds) {
        try {
          // Try to parse roleIds as JSON
          const roleIdsArray =
            typeof updatedCollaborator.roleIds === "string"
          ...
          console.log(
            `Error parsing roleIds for collaborator ${updatedCollaborator.id}: ${err}`,
            "collaborators"
          );
        }
      }
      ...
      console.log(`Error updating collaborator: ${error}`, "collaborators");
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        return res
          .status(500)
          .json({ message: "Failed to create job activity" });
      }
    }
  );
  ...
  app.put(
    "/api/jobactivities/:id",
    parseDates,
    async (req: Request, res: Response) => {
      try {
        const jobActivityData = insertJobActivitySchema
          .partial()
          .parse(req.body);
        ...
        if (
          ...
          const currentCollaboratorIds =
            await storage.getCollaboratorsByActivity(
              updatedJobActivity.activityId
            );
          ...
          for (const currentId of currentCollaboratorIds) {
            if (!req.body.collaboratorIds.includes(currentId)) {
              await storage.removeCollaboratorFromActivity(
                updatedJobActivity.activityId,
                currentId
              );
            }
        ...
        return res
          .status(500)
          .json({ message: "Failed to update job activity" });
      }
    }
  );
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        return res
          .status(500)
          .json({ message: "Failed to complete job activity" });
      }
    }
  );
    ...
    try {
      ...
      const monthlyIncome = completedJobsInMonth.reduce((sum, job) => {
        ...
        return (
          sum +
          laborCost +
          (job.materialsCost ? parseFloat(job.materialsCost) : 0)
        );
      }, 0);
  ...
  app.post("/api/subscription-plans", async (req: Request, res: Response) => {
    try {
      console.log(
        `Creating subscription plan with data: ${JSON.stringify(req.body)}`,
        "subscription"
      );
      ...
      if (
        transformedData.monthlyPrice !== undefined &&
        transformedData.monthlyPrice !== null
      ) {
        transformedData.monthlyPrice = transformedData.monthlyPrice.toString();
      }
      if (
        transformedData.yearlyPrice !== undefined &&
        transformedData.yearlyPrice !== null
      ) {
      ...
      if (error instanceof ZodError) {
        const validationError = fromZodError(error);
        console.log(
          `Subscription plan validation error: ${validationError.message}`,
          "subscription"
        );
        return res.status(400).json({ message: validationError.message });
      }

      console.log(`Error creating subscription plan: ${error}`, "subscription");
      return res
        .status(500)
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
        console.log(
          `Updating subscription plan ${id} with data: ${JSON.stringify(
            req.body
          )}`,
          "subscription"
        );
        ...
        if (
          ...
          transformedData.monthlyPrice =
            transformedData.monthlyPrice.toString();
        }
        if (
          transformedData.yearlyPrice !== undefined &&
          transformedData.yearlyPrice !== null
        ...
        const validatedData = insertSubscriptionPlanSchema
          .partial()
  ...
  app.post("/api/user-subscriptions", async (req: Request, res: Response) => {
    try {
      console.log(
        `Creating user subscription with data: ${JSON.stringify(req.body)}`,
        "subscription"
      );

      // Normalize ISO date strings to Date instances if needed
      const transformed = { ...req.body } as any;
      if (typeof transformed.startDate === "string")
        transformed.startDate = new Date(transformed.startDate);
      if (typeof transformed.endDate === "string")
        transformed.endDate = new Date(transformed.endDate);
      if (typeof transformed.lastBillingDate === "string")
        transformed.lastBillingDate = new Date(transformed.lastBillingDate);
      if (typeof transformed.nextBillingDate === "string")
        transformed.nextBillingDate = new Date(transformed.nextBillingDate);
      ...
      if (error instanceof ZodError) {
        const validationError = fromZodError(error);
        console.log(
          `User subscription validation error: ${validationError.message}`,
          "subscription"
        );
        return res.status(400).json({ message: validationError.message });
      }

      console.log(`Error creating user subscription: ${error}`, "subscription");
      return res
        .status(500)
        .json({ message: "Failed to create user subscription" });
    }
  });
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
        console.log(
          `Updating user subscription ${id} with data: ${JSON.stringify(
            req.body
          )}`,
          "subscription"
        );
        ...
        const validatedData = insertUserSubscriptionSchema
          .partial()
          .parse(transformed);
        const updatedSubscription = await storage.updateUserSubscription(
          id,
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!config) {
          return res
            .status(404)
            .json({ message: "Plan configuration not found" });
        }
  ...
  app.get(
    "/api/users/:userId/plan-configuration",
    async (req: Request, res: Response) => {
      try {
        const userId = parseInt(req.params.userId);
        ...
        if (!config) {
          return res
            .status(404)
            .json({ message: "Plan configuration not found for user" });
        }
  ...
  app.post("/api/plan-configurations", async (req: Request, res: Response) => {
    try {
      console.log(
        `Creating plan configuration with data: ${JSON.stringify(req.body)}`,
        "plan-config"
      );
      ...
      if (
        ...
        typeof transformedData.features === "object"
      ) {
        transformedData.features = JSON.stringify(transformedData.features);
      }

      const validatedData =
        insertPlanConfigurationSchema.parse(transformedData);
      ...
      if (error instanceof ZodError) {
        const validationError = fromZodError(error);
        console.log(
          `Plan configuration validation error: ${validationError.message}`,
          "plan-config"
        );
        return res.status(400).json({ message: validationError.message });
      }

      console.log(`Error creating plan configuration: ${error}`, "plan-config");
      return res
        .status(500)
        .json({ message: "Failed to create plan configuration" });
    }
  });
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
        console.log(
          `Updating plan configuration ${id} with data: ${JSON.stringify(
            req.body
          )}`,
          "plan-config"
        );
        ...
        const validatedData = insertPlanConfigurationSchema
          .partial()
          .parse(transformedData);
        const updatedConfig = await storage.updatePlanConfiguration(
          id,
          validatedData
        );
  ...
  app.post("/api/admin/login", async (req: Request, res: Response) => {
    try {
      ...
      try {
        console.log(`Checking for administrator user: ${username}`, "admin");
        const user = await storage.getUserByUsername(username);

        if (!user) {
          console.log(`User not found: ${username}`, "admin");
          return res.status(401).json({ message: "Invalid credentials" });
        }

        console.log(`User found: ${username}, type: ${user.type}`, "admin");
        ...
        const hashedPassword = crypto
          ...
          .digest("hex");
        console.log(
          `Password check - Input hash: ${hashedPassword}, Stored hash: ${user.password}`,
          "admin"
        );

        if (hashedPassword !== user.password) {
          console.log(`Password mismatch for user: ${username}`, "admin");
          return res.status(401).json({ message: "Invalid credentials" });
        }
        ...
        console.log(`Database error during admin login: ${dbError}`, "admin");
        return res.status(401).json({ message: "Invalid credentials" });
      }
    } catch (error) {
      console.log(`Admin login error: ${error}`, "admin");
  ...
  app.get("/api/clients", async (req: Request, res: Response) => {
    try {
      ...
      console.log(`Error fetching clients: ${error}`, "clients");
  ...
  app.post("/api/clients", async (req: Request, res: Response) => {
    try {
      ...
      console.log(`Error creating client: ${error}`, "clients");
  ...
  app.put("/api/clients/:id", async (req: Request, res: Response) => {
    try {
      ...
      console.log(`Error updating client: ${error}`, "clients");
  ...
  app.delete("/api/clients/:id", async (req: Request, res: Response) => {
    try {
      ...
      console.log(`Error deleting client: ${error}`, "clients");
  ...
  app.get("/api/jobs", async (req: Request, res: Response) => {
    try {
      ...
      console.log(`Error fetching jobs: ${error}`, "jobs");
  ...
  app.post("/api/jobs", async (req: Request, res: Response) => {
    try {
      ...
      console.log(`Error creating job: ${error}`, "jobs");
  ...
  app.put("/api/jobs/:id", async (req: Request, res: Response) => {
    try {
      ...
      console.log(`Error updating job: ${error}`, "jobs");
  ...
  app.delete("/api/jobs/:id", async (req: Request, res: Response) => {
    try {
      ...
      console.log(`Error deleting job: ${error}`, "jobs");
    ...
    try {
      ...
      const sectorId =
        req.query.sector === "all"
          ? null
          : parseInt(req.query.sector as string);
      const planId =
        req.query.plan === "all" ? null : parseInt(req.query.plan as string);
      ...
      const clientsByPlan = plans.map((plan) => {
        const count = filteredSubscriptions.filter(
      ...
      const subsActive = filteredSubscriptions.filter(
        (sub) => sub.status === "active" || sub.status === "trial"
      ).length;
      ...
      filteredJobs.forEach((job) => {
        if (job.status === "completed") {
          ...
          const jobTotal =
            (job.materialsCost ? parseFloat(job.materialsCost) : 0) +
            parseFloat(job.hourlyRate) * parseFloat(job.duration);
      ...
      return res
        .status(500)
        .json({ message: "Error fetching advanced statistics" });
    }
  });
  ...
  app.post(
    "/api/demo/setup-restrictive-config",
    async (req: Request, res: Response) => {
      try {
  ...
  app.get("/api/analytics", async (req: Request, res: Response) => {
    try {
      ...
      const [usersData, allJobsData, clientsData, jobTypesData] =
        await Promise.all([
      ...
      const jobsData = startDate
        ? allJobsData.filter(
            (job) => job.createdAt && new Date(job.createdAt) >= startDate!
          )
        : allJobsData;
      ...
      const totalRevenue = completedJobs.reduce((sum, job) => {
        const jobRevenue =
          parseFloat(job.hourlyRate.toString()) *
          parseFloat((job.actualDuration || job.duration).toString());
      ...
      const thisMonthJobs = jobsData.filter(
        (job) => job.createdAt && new Date(job.createdAt) >= thisMonth
      ).length;

      const lastMonthJobs = jobsData.filter(
        (job) =>
          job.createdAt &&
      ...
      completedJobs.forEach((job) => {
        ...
        if (clientRevenue.has(clientId)) {
          clientRevenue.set(
            clientId,
            clientRevenue.get(clientId) + totalJobRevenue
          );
        } else {
      ...
      subscriptions.forEach((subscription) => {
        ...
        if (plan) {
          ...
          planDistribution.set(
            planName,
            (planDistribution.get(planName) || 0) + 1
          );
        }
      });
  ...
  app.get("/api/analytics/revenue", async (req: Request, res: Response) => {
    try {
      ...
      const yearlyJobs = completedJobs.filter(
        (job) =>
          job.completedDate &&
      ...
      const lastMonthJobs = completedJobs.filter(
        (job) =>
          job.completedDate &&
      ...
      completedJobs.forEach((job) => {
        ...
        if (revenueByType.has(jobType)) {
          revenueByType.set(
            jobType,
            revenueByType.get(jobType) + totalJobRevenue
          );
        } else {
      ...
      res
        .status(500)
        .json({ error: "Errore nel recupero delle metriche di ricavo" });
    }
  });
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (existingPage) {
          return res
            .status(400)
            .json({ error: "Esiste già una pagina con questo slug" });
        }
        ...
        if (parsedBody.isHomepage) {
          ...
          const pagesOfSameType = pages.filter(
            (page) => page.type === parsedBody.type
          );
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        if (parsedBody.isHomepage) {
          ...
          const pagesOfSameType = pages.filter(
            (page) =>
              page.type === (parsedBody.type || existingWebPage.type) &&
              page.id !== parseInt(id)
          );
          for (const page of pagesOfSameType) {
            if (page.isHomepage) {
        ...
        const updatedWebPage = await storage.updateWebPage(
          parseInt(id),
          parsedBody
        );
  ...
  app.get(
    "/api/web-pages/type/:type/homepage",
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        const homepage = pages.find(
          (page) => page.isHomepage && page.status === "published"
        );

        if (!homepage) {
          return res
            .status(404)
            .json({ error: "Nessuna homepage trovata per questo tipo" });
        }
  ...
  app.post(
    "/api/setup/job-activities-table",
    async (req: Request, res: Response) => {
      try {
        const db = await initDB();
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const canRegisterActivities =
          planConfig?.features?.permissions?.["activity.register"] !== false;
        ...
        if (req.headers["content-type"]?.includes("multipart/form-data")) {
          ...
          activityData = {
            ...
            activityId:
              body.activityId !== undefined && body.activityId !== null
                ? parseInt(body.activityId)
                : undefined,
            ...
            actualDuration: body.actualDuration
              ? parseFloat(body.actualDuration)
              : null,
            notes: body.notes || null,
            materials: body.materials || null,
            materialsCost: body.materialsCost
              ? parseFloat(body.materialsCost)
              : null,
        ...
        const newJobActivity = await storage.createJobActivity(
  ...
  app.get("/api/admin/web-pages", async (req: Request, res: Response) => {
    try {
      ...
      console.log(`Error fetching web pages: ${error}`, "web-page");
  ...
  app.get("/api/admin/web-pages/:id", async (req: Request, res: Response) => {
    try {
      ...
      console.log(`Error fetching web page: ${error}`, "web-page");
  ...
  app.post("/api/admin/web-pages", async (req: Request, res: Response) => {
    try {
      console.log(
        `Creating web page with data: ${JSON.stringify(req.body)}`,
        "web-page"
      );
      ...
      const transformedData = { ...req.body };
      if (
        transformedData.authorId !== undefined &&
        transformedData.authorId !== null
      ) {
      ...
      if (error instanceof ZodError) {
        console.log(`Web page validation error: ${error}`, "web-page");
        return res.status(400).json({
          error: "Dati della pagina web non validi",
          details: error.flatten(),
        });
      }
      console.log(`Error creating web page: ${error}`, "web-page");
  ...
  app.put("/api/admin/web-pages/:id", async (req: Request, res: Response) => {
    try {
      ...
      const validatedData = insertWebPageSchema
        .partial()
        .parse(transformedData);
      ...
      console.log(`Error updating web page: ${error}`, "web-page");
  ...
  app.delete(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(`Error deleting web page: ${error}`, "web-page");
  ...
  app.get("/api/reports/performance", async (req: Request, res: Response) => {
    try {
      ...
      const revenue = jobs.reduce(
        (sum, job) =>
          sum + parseFloat(job.hourlyRate) * parseFloat(job.duration),
        0
      );

      // Calculate time data
      const userJobIds = jobs.map((job) => job.id);
      const userActivities = jobActivities.filter((activity) =>
        userJobIds.includes(activity.jobId)
      );

      const totalPlanned = userActivities.reduce(
        (sum, activity) => sum + parseFloat(activity.duration),
        0
      );
      const totalActual = userActivities.reduce(
        (sum, activity) =>
          sum + parseFloat(activity.actualDuration || activity.duration),
        0
      );
      const efficiency =
        totalPlanned > 0 ? (totalPlanned / totalActual) * 100 : 0;

      // Calculate cost data
      const totalCosts = jobs.reduce(
        (sum, job) =>
          sum + (job.materialsCost ? parseFloat(job.materialsCost) : 0),
        0
      );
      const materialsCost = jobs.reduce(
        (sum, job) =>
          sum + (job.materialsCost ? parseFloat(job.materialsCost) : 0),
        0
      );
      ...
      const onTimeCompletion =
        completedJobs > 0 ? (completedJobs / jobs.length) * 100 : 0;
      ...
      res.json({
        ...
        performanceData: {
          score: Math.round(
            (efficiency + onTimeCompletion + clientSatisfaction) / 3
          ),
      ...
      res
        .status(500)
        .json({ error: "Errore nel recuperare i dati di performance" });
    }
  });
    ...
    try {
      ...
      const byClient = clients
        .map((client) => {
          ...
          const clientRevenue = clientJobs.reduce(
            (sum, job) =>
              sum + parseFloat(job.hourlyRate) * parseFloat(job.duration),
            0
          );
      ...
      res
        .status(500)
        .json({ error: "Errore nel recuperare i dati finanziari" });
    }
  });
  ...
  app.get("/api/reports/time", async (req: Request, res: Response) => {
    try {
      ...
      const byJobType = jobs.reduce((acc, job) => {
        ...
        const jobActivities = userActivities.filter(
          (activity) => activity.jobId === job.id
        );
        const planned = jobActivities.reduce(
          (sum, activity) => sum + parseFloat(activity.duration),
          0
        );
        const actual = jobActivities.reduce(
          (sum, activity) =>
            sum + parseFloat(activity.actualDuration || activity.duration),
          0
        );
      ...
      res
        .status(500)
        .json({ error: "Errore nel recuperare i dati di analisi temporale" });
    }
  });
  ...
  app.get("/api/reports/efficiency", async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Errore nel recuperare i dati di efficienza" });
    }
  });
